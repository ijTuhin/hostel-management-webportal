...
const TableData = ({ item, data }) => {
  const abc = data;
  const cancelSeat = () => {
    fetch(`http://localhost:3001/seat/${item?.room?._id}/remove/${item?._id}`, {
      method: "PUT",
      headers: {
    ...
    window.onload(alert(`Removed User ${item?.matric} from Room ${item?.room?._id}`));
  };
  const fetchRoomData = () => {
  ...
  return (
    <>
      <tr class="bg-white border-2 border-l-0 border-r-0 border-zinc-300 transition duration-300 ease-in-out text-gray-900">
        ...
        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
          {item.room?.room ? <>{item.room.room}</> : "N/A"}
        </td>
